...
// 商品表
type Goods struct {
	GoodsId        int64   `gorm:"primary_key" json:"goods_id"`
	...
	GoodsWeight    int     `gorm:"default:0" json:"goods_weight"`     // 商品重量
	CategoryId     uint64  `json:"category_id"`                       // 类型id
	GoodsIntroduce string  `gorm:"type:text" json:"goods_introduce"`  // 商品详情介绍
	GoodsBigLog    string  `gorm:"default:''" json:"goods_big_log"`   // 图片logo大图
	GoodsSmallLog  string  `gorm:"default:''" json:"goods_small_log"` // 图片logo小图
	IsDel          int     `gorm:"default:0" json:"is_del"`           // 0:正常  1:删除
	AddTime        int     `gorm:"size:11" json:"add_time"`
	UpdTime        int     `gorm:"size:11;NULL" json:"upd_time"`
	DeleteTime     int     `gorm:"size:11;NULL" json:"delete_time"`       // 软删除标志字段
...
type GoodsAttr struct {
	...
	AttributeId int64   `json:"attribute_id"`                           // 属性id
...
type GoodsPics struct {
	PicsId  int    `gorm:"primary_key" json:"pics_id"`
	GoodsId int64  `json:"goods_id"`                   // 商品id
...
type GoodsCats struct {
	CatId      int64  `gorm:"primary_key" json:"cat_id"` // 分类id
